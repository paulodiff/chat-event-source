...
//function initReliableSignaler(connection, socketURL, fullURL, relativePath) {

function initReliableSignaler(connection, socket) {
    //var socket;
    
    //fullURL = "https://pmlab.comune.rimini.it";
    //relativePath = "es2/socket.io"


    //console.log('signaler.js io.connect:socketURL:', socketURL);
    //console.log('signaler.js io.connect:fullURL:', fullURL);
    //console.log('signaler.js io.connect:relativePath:', relativePath);
    
    
    if (!connection) throw '"connection" argument is required.';

    function initSocket() {
        console.log('ReliableSignaler:initSocket');
        ...
        //socket = io.connect(fullURL, {resource: relativePath});
        ...
        socket.on('error', function() {
            console.log('ReliableSignaler:error');
            socket.isHavingError = true;
            initSocket();
        });

        socket.on('disconnect', function() {
            console.log('ReliableSignaler:disconnect');
            socket.isHavingError = true;
            initSocket();
        });
    }
