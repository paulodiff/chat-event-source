...
.controller('displayCtrl', ['$scope', '$timeout', '$log', 'ENV' , 'HelloWorldService','$ionicPopup','$window',
                       function ($scope,   $timeout,   $log,   ENV,    HelloWorldService,  $ionicPopup, $window) {
    
    $log.debug("displayCtrl ... start ..join connection 9999");
   ...
   // initializing RTCMultiConnection.js constructor.
    ...
    connection.onopen = function(ev) {
        console.log('open! connection!', ev);
    };

    //channel.onmessage = appendDIV;
    connection.onmessage = manageData;
    connection.onerror = logError;
    connection.onclose = logClose;

    connection.openOrJoin('9999');
    ...
    function manageData(message){
  
      //console.log('logData',message);
      //console.log('logData',message.extra);
      //$scope.userData.workerInfo = message.data.value + "/" + message.latency;
      //$scope.$apply();
      //if(msg.data.action == "move"){ movePaddle(msg); }
      //if(msg.data.action == "start"){ startGame(); }
      //addLogDIV(message.data.value);
      console.log(message);

      if (message.data.action == "move"){
        if (message.data.player == "P1") {
            paddles[1].x = message.data.value - paddles[1].w/2;
            var p = paddles[1];
        } else {
            paddles[2].x = message.data.value - paddles[2].w/2;
            var p = paddles[2];
        }
        //paintCanvas();
        ctx.fillStyle = "white";
...
var canvas = document.getElementById("canvas"),
    ...
    Warea = area.clientWidth,
    Harea = area.clientHeight,
...
function movePaddle(X){
  ...
  console.log('movePaddle', X.data.player, X.data.value);

  paintCanvas();

  paddles[1].x = X.data.value - paddles[1].w/2;
